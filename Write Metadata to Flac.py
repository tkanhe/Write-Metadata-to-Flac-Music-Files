...
def setup(err=3, sizes=1000):
    for j in files:
        ...
        try:
            ...
            if xx:
                x1 = [i for i in xx if
                      (i['kind'] == 'song') and (abs(i['trackTimeMillis'] / 1000 - audio.info.length) <= err)]
            else:
                continue
            ...
            x = [[i['trackCensoredName'], i['artistName'], i['collectionName'], i['primaryGenreName'],
                  i['releaseDate'][:10], i['artworkUrl100']] for i in x1[:4]]
